...
import { BlockCollector } from '../lib/collectors';

const help = `
Parses the indicated Javascript or Typescript files for comment blocks and
outputs them as .json files of the same name in the same directory.

Syntax: ts-node gen_block_json.ts <fileName>+ [--log]
...
fileNames.forEach(fileName => {
    ...
    const collector = new BlockCollector(sourceText, args.log);
